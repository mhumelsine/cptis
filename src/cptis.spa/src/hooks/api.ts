...
export interface ApiEndpoint<TGet, TSearch, TCreated>
    extends ListEndpoint<TSearch>, GetEndpoint<TGet> {
    post: (body: any, queryParam?: any) => Promise<TCreated>;
    put: (id: number | string, body: any, queryParam?: any) => Promise<TCreated | void>;
    patch: (id: number | string, patchDoc: any, queryParam?: any) => Promise<void>;
...
export class AbortableRestApi<TGet, TSearch, TCreated> implements ApiEndpoint<TGet, TSearch, TCreated> {
    ...
    public post = (body: any, queryParam?: any) =>
        this.sendBody<TCreated>('POST', this.createRoute(undefined, queryParam), body);
    ...
    private async sendBody<T>(method: 'POST' | 'PUT' | 'DELETE' | 'PATCH', url: string, body: any) {
        const { key, signal } = this.createAbortController();
